...
pub fn parse<'a>(files: impl Iterator<Item = &'a File<'a>>) -> Vec<Vec<&'a str>> {
    let files = files.collect::<Vec<_>>();
    let mut parsed_files: Vec<Vec<&str>> = Vec::with_capacity(files.len());
    ...
    for file in files {
        ...
        let mut parsed_sections: Vec<&str> = Vec::with_capacity(SDO_SECTIONS.len());

        for section in SDO_SECTIONS {
            let (remaining, parsed_section) = content.split_once(section).unwrap();

            parsed_sections.push(parsed_section.trim());
            content = remaining;
        }
