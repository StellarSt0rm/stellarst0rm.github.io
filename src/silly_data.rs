...
pub fn parse_sdo<'a>(files: impl Iterator<Item = &'a File<'a>>) -> Vec<Vec<String>> {
    ...
    for file in files {
        ...
        for section in SDO_SECTIONS {
            ...
            let mut parsed_section = parsed_section.to_string();

            // Markdown automatically escapes dangerous html, so its safe.
            if section == "Content:" {
                parsed_section =
                    markdown::to_html_with_options(&parsed_section, &markdown::Options::gfm())
                        .unwrap();
            }
